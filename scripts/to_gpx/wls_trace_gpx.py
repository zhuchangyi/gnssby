...
def add_csv_to_gpx(csv_filename, gpx_trkseg, doc):
    with open(csv_filename, newline='', encoding='ISO-8859-1') as csvfile:
        ...
        for row in csvreader:
            ...
            trkpt.setAttribute("lat", str(row["WlsLatitudeDegrees"]))
            trkpt.setAttribute("lon", str(row["WlsLongitudeDegrees"]))
...
def csv_to_gpx_thread(csv_filename):
    ...
    gpx_filename = os.path.join(os.path.dirname(csv_filename), f"wls_{date_folder}_{phone_name}.gpx")
