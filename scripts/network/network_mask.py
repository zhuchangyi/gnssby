...
class GNSSClassifier(nn.Module):
    def __init__(self, d_model=32, nhead=4, d_ff=64, num_layers=3, num_satellites=81, num_classes=22, dropout=0.1):
    ...
    def apply_mask(self, output):
        batch_size, num_classes = output.shape
        mask = torch.zeros_like(output)
        max_indices = torch.argmax(output, dim=1, keepdim=True)
        positive_mask = (max_indices < 11).float()
        negative_mask = 1 - positive_mask

        mask[:, :11] = positive_mask
        mask[:, 11:] = negative_mask
...
class GNSSRegressor(nn.Module):
    def __init__(self, d_model=32, nhead=4, d_ff=64, num_layers=2, num_satellites=81):
        super(GNSSRegressor, self).__init__()
        self.input_proj = nn.Linear(15, d_model)  # 初始投影层
        self.transformer_encoder = TransformerEncoder(d_model, nhead, d_ff, num_layers)  # Transformer编码器
        self.regressor = nn.Linear(d.model * num_satellites, 3)  # 回归器层，输出三个值(x, y, z)
